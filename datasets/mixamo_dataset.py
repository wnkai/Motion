...
class MIXAMOData(data.Dataset):
    def __init__(self, args, character_name):
        super(MIXAMOData, self).__init__()
        file_path = './datasets/Mixamo/{}.npy'.format(character_name)

        print('load from file {}'.format(file_path))
        self.total_frame = 0
        self.std_bvh = get_std_bvh(args, dataset=character_name)
        file = BVH_file(self.std_bvh)
        self.joint_topologies =file.topology
        self.ee_ids = file.get_ee_id()
        self.edges = build_edge_topology(self.joint_topologies, torch.zeros((len(self.joint_topologies), 3)))

        self.offset = file.offset
        self.offset = torch.tensor(self.offset, dtype=torch.float)
        ...
        new_windows = self.make_windows(motions)
    ...
    def make_windows(self, motions):
        ...
        for motion in motions:
            ...
            step_size = self.args.windows_size // 2
